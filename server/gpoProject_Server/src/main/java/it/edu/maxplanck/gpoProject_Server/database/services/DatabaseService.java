...
public class DatabaseService {
	 ...
	 * @throws DatabaseException
	 */
	public ResponseMessageChatDTO createAttachmentChat(int id, Integer chatId, List<String> filename, List<String> extension, String message) throws DatabaseException {
		// TODO Auto-generated method stub
		...
		for(int i = 0; i < filename.size(); i++) {
            ...
            if(filename.get(i).length() <= UtilDatabase.AttachedData.filenameLenght && extension.get(i).length() <= UtilDatabase.AttachedData.extensionLenght){
               a =  this.attachmentsRepo.save(new Attached(mc, filename.get(i), extension.get(i)));
            }
            if(a!=null){
                System.out.println("/files/"+a.getFilename()+a.getExtension().replace(".", "z"));
                attachments.add(new ResponseAttachedChatDTO(a.getPkID(), "/files/"+a.getFilename()+a.getExtension(), a.getFilename(), a.getExtension()));
            }
