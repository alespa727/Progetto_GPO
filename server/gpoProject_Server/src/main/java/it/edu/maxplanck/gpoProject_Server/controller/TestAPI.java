...
public class TestAPI extends BasicApiRestController {
	...
	public TestAPI(TokenService tokenService, CookieService cookieService) {
		super();
	...
	// if(subject == null) return respons = new ResponseEntity<String>(HttpStatus.INTERNAL_SERVER_ERROR);
		
		Map<String, Object> claims = new HashMap<String, Object>();
		
		claims.put("username", "ale");
		claims.put("password", "password");
	// if(claims == null) return respons = new ResponseEntity<String>(HttpStatus.INTERNAL_SERVER_ERROR);
		
		token = this.tokenService.getTokenAccess(subject, claims);
	// if(token == null) return respons = new ResponseEntity<String>(HttpStatus.INTERNAL_SERVER_ERROR);
		
		String name = "CookieTest";
		Cookie cookie = this.cookieService.generateCookie(name, token, true, false, "/api/", 15 * UtilServer.timeExpirationDateCookieToken);
