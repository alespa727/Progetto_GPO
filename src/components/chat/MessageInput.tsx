...
export function MessageInput({ addMessage, forceUpdate }: { addMessage: (message: Message) => void, forceUpdate: React.Dispatch<React.SetStateAction<number>> }) {
    ...
    const setChats = useChats()?.setChats;
    ...
    if (!setChats || !chat || !(account && account !== undefined)) return;
    ...
    const sendMessageAndAttachments = async (nuovoTesto: string, files: File[]) => {
        ...
        try {
            ...
            const res = await axios.post<Message>(endpoint2 + "/services/chats/" + chat.id + "/attachments", formData, {
                withCredentials: true,
            });
    ...
    return (
        <div className="pb-1.5 pr-1.5 pl-1.5 min-w-0">
