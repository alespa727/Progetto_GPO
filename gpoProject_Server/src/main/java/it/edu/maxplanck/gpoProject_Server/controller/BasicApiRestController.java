...
public abstract class BasicApiRestController {
	...
	protected String findImage(User u) {
		...
		if (!Files.exists(imagePath)) {
			...
			this.databaseService.updateUserProfile(u.getId(), null);
		} else {
	 ...
	 * @return
	 */
	protected String findAttachment(AttachedChat a) {
		String path = null;
		if(a.getFilename() != null && a.getExtension() != null) {
			...
			if (!Files.exists(imagePath)) {
				// immagine persa → reset DB
				this.databaseService.deleteAttached(a.getId());
			} else {
				path = "";
			}
		}
    ...
    protected String findAttachmentCommunity(AttachedCommunity a) {
        String path = null;
        if(a.getFilename() != null && a.getExtension() != null) {
