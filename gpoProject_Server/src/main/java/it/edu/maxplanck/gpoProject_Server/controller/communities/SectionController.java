...
public class SectionController  extends BasicApiRestController {
    ...
    public ResponseEntity<?> postSection(HttpServletRequest request, HttpServletResponse response, @PathVariable("communityId") Integer communityId, @RequestBody RequestSectionDTO body) {
        ...
        List<Channel> channelList = this.databaseService.getChannelsRepo().findChannelsByFkSection(newSection);
        List<ResponseChannelDTO> channelDTOList = new ArrayList<>();
        for (Channel channel : channelList) {
            channelDTOList.add(new ResponseChannelDTO(channel.getPkID(), channel.getName(), channel.getType(), channel.getDescription(), channel.getCreatedAt()));
        }
        ...
        return ResponseEntity.created(location).body(new ResponseSectionDTO(newSection.getPkID(), newSection.getName(), channelDTOList));
    }
